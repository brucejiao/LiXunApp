...
import com.yuzhi.lixun110ccd.model.GoogleLoc2Add.GoogleAddressComponents;
import com.yuzhi.lixun110ccd.model.GoogleLoc2Add.GoogleLoc;
import com.yuzhi.lixun110ccd.model.GoogleLoc2Add.GoogleResults;
import com.yuzhi.lixun110ccd.model.IssueModel.IssueContent;
...
import com.yuzhi.lixun110ccd.ui.SpinnerArrayAdapter;
...
import static com.yuzhi.lixun110ccd.utils.CommUtil.getAddressId;
import static com.yuzhi.lixun110ccd.utils.CommUtil.readAssert;
import static com.yuzhi.lixun110ccd.utils.CommUtil.showEditString;
...
import static com.yuzhi.lixun110ccd.utils.Constant.PARENTID_WLQZHU;
import static com.yuzhi.lixun110ccd.utils.Constant.PARENTID_WTXR;
import static com.yuzhi.lixun110ccd.utils.Constant.PARENTID_WTXW;
import static com.yuzhi.lixun110ccd.utils.Constant.PARENTID_ZLRL;
 ...
 * 编辑草稿箱
 */
public class EditDraftActivity extends AppCompatActivity implements OnAddressChangeListener {
    ...
    private boolean isCkeckedPicture = false ;//是否选择图片
...
//                showToast(title,mContext);
    ...
    private void initData(){
        ...
        FindListBean bean=(FindListBean)getIntent().getSerializableExtra("bean");
        switch (parentID){
            case PARENTID_WLBG:
         ...
         String userID = share.getString(SHARE_LOGIN_USERID, "");// 用户Id
            //发布信息实体
               ...
               LogUtil.d("=============params=============",publishinfo+"----\n----"+toJSONString(mPicturelists));
                HttpClient.get(Caller.MODIFY_PUBLISH_INFO,params, new HttpResponseHandler() {
                    @Override
                    public void onSuccess(RestApiResponse response) {
                        String     result   =  response.getResult();
                        String     message  =  response.getMessage();
...
//             showToast("imgLIST===>"+ toJSONString(mPicturelists),mContext);
        ...
        //        params.put("provepicturelist","=====");//发布证明图片信息实体
        LogUtil.d("=============params=============",publishinfo+"----\n----"+toJSONString(mPicturelists));
        HttpClient.get(Caller.MODIFY_SUBMIT_PUBLISH_INFO,params, new HttpResponseHandler() {
            @Override
            public void onSuccess(RestApiResponse response) {
                String     result   =  response.getResult();
                String     message  =  response.getMessage();
    ...
    private void initDraftData(){
        ...
        Picasso.with(mContext).load(bean.getImgFilePath())
                .resize(DeviceUtil.dp2px(mContext,35), DeviceUtil.dp2px(mContext,35))
                .placeholder(R.drawable.default_image).into(mIssueUserHeader);//用户头像
        mIssueTitle.setText(bean.getTitle());//发布标题
        mIssueContent.setText(bean.getContent());//发布内容
//        mIssueSecondType.s   //二级菜单（类型）
        mIssuePrice.setText(bean.getMoney());//悬赏金额
        mIssueCity.setText(bean.getProvinceName()+" "+bean.getCityName());//走失城市
        mIssueDetailAddress.setText(bean.getAddress());//详细地址

       String publishID =  bean.getPublishID();
        getDetailsInfos(publishID);
     ...
     * 根据pushID获取发布消息详情
     */
    private void getDetailsInfos(String publishID) {
        final List<String>  picList = new ArrayList<String>();
        mPicturelists = new ArrayList<Picturelist>();
...
//        progress = CommUtil.showProgress(mContext, "正在加载数据，请稍候...");
        ...
        HttpClient.get(Caller.DETAILS_ISSUE_INFOS, params, new HttpResponseHandler() {
            ...
            public void onSuccess(RestApiResponse response) {
                ...
                if (!CommUtil.isNullOrBlank(result) && result.equals(RESUTL_TRUE)) {
                    ...
                    for (int index = 0; index < findListPicListsNum; index++) {
                        ...
                        String picId = findListPicLists.get(index).getPictureID();
                        picList.add(picDetails);
                        Picturelist  picture = new Picturelist();
                        picture.setPictureID(picId);
                        mPicturelists.add(picture);

                    }
                    if(findListPicListsNum == picList.size()){
                        choicePicture(findListPicListsNum,picList);
                    }
            ...
            @Override
            public void onFailure(Request request, Exception e) {
                showToast("草稿箱图片列表获取失败", mContext);
                if (progress != null) {
                    progress.dismiss();
                }
            }
    ...
    private void choicePicture(int flag,List<String>  picList){
        switch(flag){
            case 1:
                Picasso.with(mContext).load(picList.get(0))
                        .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                        .placeholder(R.drawable.default_image).into(mIssueImgOne);//图片一
                mIssueImgOne.setVisibility(View.VISIBLE);
                mIssueImgTwo.setVisibility(View.VISIBLE);
                mIssueImgThree.setVisibility(View.GONE);
                mIssueImgFour.setVisibility(View.GONE);
            ...
            case 2:
                Picasso.with(mContext).load(picList.get(0))
                        .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                        .placeholder(R.drawable.default_image).into(mIssueImgOne);//图片一
                Picasso.with(mContext).load(picList.get(1))
                        .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                        .placeholder(R.drawable.default_image).into(mIssueImgTwo);//图片二
                mIssueImgOne.setVisibility(View.VISIBLE);
                mIssueImgTwo.setVisibility(View.VISIBLE);
                mIssueImgThree.setVisibility(View.VISIBLE);
                mIssueImgFour.setVisibility(View.GONE);
            ...
            case 3:
                ...
                Picasso.with(mContext).load(picList.get(1))
                        .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                        .placeholder(R.drawable.default_image).into(mIssueImgTwo);//图片二
                Picasso.with(mContext).load(picList.get(2))
                        .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                        .placeholder(R.drawable.default_image).into(mIssueImgThree);//图片三
                mIssueImgOne.setVisibility(View.VISIBLE);
                mIssueImgTwo.setVisibility(View.VISIBLE);
                mIssueImgThree.setVisibility(View.VISIBLE);
                mIssueImgFour.setVisibility(View.VISIBLE);
            ...
            case 4:
                ...
                Picasso.with(mContext).load(picList.get(2))
                        .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                        .placeholder(R.drawable.default_image).into(mIssueImgThree);//图片三
                Picasso.with(mContext).load(picList.get(3))
                        .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                        .placeholder(R.drawable.default_image).into(mIssueImgFour);//图片四
                mIssueImgOne.setVisibility(View.VISIBLE);
                mIssueImgTwo.setVisibility(View.VISIBLE);
                mIssueImgThree.setVisibility(View.VISIBLE);
                mIssueImgFour.setVisibility(View.VISIBLE);
            ...
            case 5:
                ...
                Picasso.with(mContext).load(picList.get(3))
                        .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                        .placeholder(R.drawable.default_image).into(mIssueImgFour);//图片四
                Picasso.with(mContext).load(picList.get(4))
                        .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                        .placeholder(R.drawable.default_image).into(mIssueImgFive);//图片五
                mIssueImgOne.setVisibility(View.VISIBLE);
                mIssueImgTwo.setVisibility(View.VISIBLE);
                mIssueImgThree.setVisibility(View.VISIBLE);
                mIssueImgFour.setVisibility(View.VISIBLE);
            ...
            case 6:
                ...
                Picasso.with(mContext).load(picList.get(4))
                        .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                        .placeholder(R.drawable.default_image).into(mIssueImgFive);//图片五
                Picasso.with(mContext).load(picList.get(5))
                        .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                        .placeholder(R.drawable.default_image).into(mIssueImgSix);//图片六
                mIssueImgOne.setVisibility(View.VISIBLE);
                mIssueImgTwo.setVisibility(View.VISIBLE);
                mIssueImgThree.setVisibility(View.VISIBLE);
                mIssueImgFour.setVisibility(View.VISIBLE);
            ...
            case 7:
                ...
                Picasso.with(mContext).load(picList.get(5))
                        .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                        .placeholder(R.drawable.default_image).into(mIssueImgSix);//图片六
                Picasso.with(mContext).load(picList.get(6))
                        .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                        .placeholder(R.drawable.default_image).into(mIssueImgSeven);//图片七
                mIssueImgOne.setVisibility(View.VISIBLE);
                mIssueImgTwo.setVisibility(View.VISIBLE);
                mIssueImgThree.setVisibility(View.VISIBLE);
                mIssueImgFour.setVisibility(View.VISIBLE);
        ...
        Picasso.with(mContext).load(picList.get(6))
                .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                .placeholder(R.drawable.default_image).into(mIssueImgSeven);//图片七
        Picasso.with(mContext).load(picList.get(7))
                .resize(DeviceUtil.dp2px(mContext,73), DeviceUtil.dp2px(mContext,73))
                .placeholder(R.drawable.default_image).into(mIssueImgEight);//图片八
                mIssueImgOne.setVisibility(View.VISIBLE);
                mIssueImgTwo.setVisibility(View.VISIBLE);
                mIssueImgThree.setVisibility(View.VISIBLE);
                mIssueImgFour.setVisibility(View.VISIBLE);
